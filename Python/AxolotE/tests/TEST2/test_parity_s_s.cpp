...
bool run_test_l0_l0() {
    ...
    std::vector<double> E_orig = ECoeff0And0(alpha, beta, A_orig, B_orig);
    std::vector<double> E_inv  = ECoeff0And0(alpha, beta, A_inv,  B_inv);
    ...
    for (size_t i = 0; i < E_orig.size(); ++i) {
        ...
        if (std::abs(mag_orig - mag_inv) > 1e-12) {
            std::cerr << "[FAIL] Parity magnitude broken for l=" << 0 << ", lp=" << 0 
                      << " at index " << i 
