...
(*maxLp=2;*)
(*Nvals =0;*)
(*(*Store current file location path*)*)
(*CurrentDirectory=If[$Notebooks,NotebookDirectory[],Directory[]];*)
...
(*ECoefFile=OpenWrite[FileNameJoin[{CurrentDirectory,"AxolotE_Mat.cpp"}]];(*Use OpenAppend to write to an existing file. Remember to specify correct file name*)
...
(**)
(*(*Write the header section of the Cpp code*)*)
(*WriteString[ECoefFile,"#include \"AxolotE_Mat.h\"*)
(*#include <iostream>*)
(*#include <vector>*)
(*#include <array>*)
(*#include <cmath>*)
...
(*For[lp=0,lp<=maxLp,lp+=1,*)
(*Nvals =0;*)
(*CurrentFunctionName=StringJoin[{"std::vector<double> ",BaseName,ToString[l],"And",ToString[lp],"(double expAlpha, double expBeta,         \[IndentingNewLine]\tstd::array<double,3> muCoords, std::array<double,3> nuCoords)"}];*)
(*AppendTo[FunctionNames,CurrentFunctionName];*)
...
(* ::Input:: *)
(*HeaderFile=OpenWrite[FileNameJoin[{CurrentDirectory,"AxolotE_Mat.h"}]];*)
(*WriteString[HeaderFile,"#ifndef ECOEFMA_H*)
(*#define ECOEFMA_H*)
(**)
(*#include <array>\n\n"]*)
(*For[i=1,i<=Length[FunctionNames],i+=1,WriteString[HeaderFile,FunctionNames[[i]],";\n"];]*)
(*WriteString[HeaderFile,"#endif"]*)
(*Close[HeaderFile]*)
