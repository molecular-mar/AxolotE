...
bool run_test_l2_l1() {
    ...
    std::vector<double> E_orig  = ECoeff2And1(alpha, beta, mu_orig, nu_orig);
    std::vector<double> E_shift = ECoeff2And1(alpha, beta, mu_shift, nu_shift);

    if (E_orig.empty() || E_orig.size() != E_shift.size()) {
        std::cerr << "[FAIL] Size mismatch or empty vector for l=" << 2 << ", lp=" << 1 << "\n";
        return false;
    }

    for (size_t i = 0; i < E_orig.size(); ++i) {
        if (std::abs(E_orig[i] - E_shift[i]) > 1e-12) {
            std::cerr << "[FAIL] Invariance broken for l=" << 2 << ", lp=" << 1 
                      << " at index " << i << "\n";
            return false;
        }
    }

    std::cout << "[PASS] Shell Pair (" << 2 << ", " << 1 << ") | Tested " 
