...
bool run_test_l0_l0() {
    ...
    std::vector<double> E_orig  = ECoeff0And0(alpha, beta, mu_orig, nu_orig);
    std::vector<double> E_shift = ECoeff0And0(alpha, beta, mu_shift, nu_shift);

    if (E_orig.empty() || E_orig.size() != E_shift.size()) {
        std::cerr << "[FAIL] Size mismatch or empty vector for l=" << 0 << ", lp=" << 0 << "\n";
        return false;
    }

    for (size_t i = 0; i < E_orig.size(); ++i) {
        if (std::abs(E_orig[i] - E_shift[i]) > 1e-12) {
            std::cerr << "[FAIL] Invariance broken for l=" << 0 << ", lp=" << 0 
                      << " at index " << i << "\n";
            return false;
        }
    }

    std::cout << "[PASS] Shell Pair (" << 0 << ", " << 0 << ") | Tested " 
