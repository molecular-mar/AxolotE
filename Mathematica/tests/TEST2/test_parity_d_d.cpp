...
bool run_test_l2_l2() {
    ...
    std::vector<double> E_orig = ECoeff2And2(alpha, beta, A_orig, B_orig);
    std::vector<double> E_inv  = ECoeff2And2(alpha, beta, A_inv,  B_inv);
    ...
    for (size_t i = 0; i < E_orig.size(); ++i) {
        ...
        if (std::abs(mag_orig - mag_inv) > 1e-12) {
            std::cerr << "[FAIL] Parity magnitude broken for l=" << 2 << ", lp=" << 2 
                      << " at index " << i 
